...
export function EditModeToggle() {
  const { isAdmin, editMode, setEditMode, hasDirtyBlocks, dirtyBlocks, saveStatus } = useEditMode()
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex items-center gap-2">
      {editMode && saveStatus !== 'idle' && (
        <span
          className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-medium shadow-lg backdrop-blur-xl border ${
            saveStatus === 'error'
              ? 'bg-error/10 border-error/30 text-error'
              : saveStatus === 'saving'
                ? 'bg-nz-bg-elevated border-nz-border text-nz-text-secondary'
